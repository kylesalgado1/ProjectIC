...
try {
  if (localStorage.getItem('ic-version') !== DATA_VERSION) {
    ['ic-packages', 'ic-promos', 'ic-rooms'].forEach((k) => localStorage.removeItem(k));
...
interface LocKnow { id: string; city: string; packages: KPkg[]; promos: KPromo[]; rooms: KRoom[]; }
interface Knowledge { currentId: string; locations: LocKnow[]; }

/* Build the multi-location knowledge the assistant reasons over. */
function buildLocations(pk: Record<string, Pkg[]>, pr: Record<string, Promo[]>, rm: Record<string, RoomType[]>): LocKnow[] {
    ...
    <header className="topbar">
      <div className="wrap topbar__inner">
        <button className="brand" onClick={() => setPage('landing')} aria-label="IC home">
          ...
          <span className="brand__text" style={{ display: 'block', textAlign: 'left' }}>
            I See
            <span className="brand__tag" style={{ display: 'block' }}>
    Every location's truth
            </span>
          </span>
...
function Dashboard() {
  ...
  const resetDemo = () => { setPackagesByLoc(SEED_PACKAGES); setPromosByLoc(SEED_PROMOS); setRoomsByLoc(SEED_ROOMS); setPkgDur({}); };
  ...
  const knowledge: Knowledge = { currentId: loc, locations: buildLocations(packagesByLoc, promosByLoc, roomsByLoc) };
        ...
        <div className="dash__col">
          <div className="card assistant">
            ...
            <Chat
              key={loc}
              knowledge={knowledge}
              welcome={`I see — ask me about packages, arcade promotions, or rooms for ${loco.name}.`}
              suggestions={['What packages do we offer?', 'Is My Melody active in Roseville?', 'Which location fits 50 guests?']}
            />
          </div>
        </div>
      </div>
    ...
    <main className="cust wrap">
      ...
      <div className="card cust__card">
        ...
        <Chat
          key={loc}
          knowledge={knowledge}
          welcome={`Welcome to ${loco.name}! I see — ask me about our packages, arcade promotions, or rooms.`}
          suggestions={['What packages do you offer?', 'Which location has a room for 50?', 'Is My Melody active in Concord?']}
        />
      </div>
    </main>
  );
